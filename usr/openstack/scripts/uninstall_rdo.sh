...
yum remove -y nrpe puppet erlang novnc openvswitch "*nagios*" "*rabbitmq*" "*redis*" "*libvirt*" "*openstack*" "*nova*" "*keystone*" "*glance*" "*cinder*" "*swift*";
...
rm -rf  /etc/my* /etc/rabbitmq /etc/redis* /etc/nagios  /etc/mysql /root/.my.cnf /etc/yum.repos.d/packstack_* /var/lib/cinder/ /var/lib/glance /var/lib/nova /etc/nova /srv/node/device*/*  /etc/rsync.d/frag* /var/lib/mysql/ /etc/swift \
